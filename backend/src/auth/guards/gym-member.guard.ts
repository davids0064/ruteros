import { CanActivate, ExecutionContext, Injectable } from '@nestjs/common';
import { DomainException } from '../../common/errors/domain.exception';
import { MembershipService } from '../membership.service';
import { isMemberOf, type AuthenticatedUser } from '../jwt-payload';

/** 04 §4.2: `:gymId` ∈ `memberships`, con respaldo en BD (ver MembershipService). */
@Injectable()
export class GymMemberGuard implements CanActivate {
  constructor(private readonly memberships: MembershipService) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    const req = context.switchToHttp().getRequest();
    const user = req.user as AuthenticatedUser | undefined;
    const gymId: string = req.params.gymId;

    if (!user) throw new DomainException('UNAUTHENTICATED', 'Token ausente, expirado o inválido.');
    if (user.role === 'admin') return true;
    if (isMemberOf(user, gymId)) return true;
    if (await this.memberships.isAuthorizedMember(user.id, gymId)) return true;

    throw new DomainException('NOT_GYM_MEMBER', 'No eres miembro autorizado de este boulder.');
  }
}
